...
6. **Element-edge display (3 modes) + view controls.** `Shaded` / `ShadedEdges`
   ...
   - **Pan vs. rotate.** Left-drag uses the configured mode (`panMode`, set by
     the on-screen "Drag: Rotate/Pan" button + `R`/`P` shortcuts); **right-drag
     does the opposite** (`MouseArea` accepts both buttons, records which started
     the drag, and inverts `panMode` for the right button). Routes to
     `ViewerItem::pan()` / `orbit()`. `TrackballCamera::pan()` shifts the target
     in the view plane (right/up from orientation), scaled by distance so it
     feels consistent at any zoom. Button highlight tracks `panMode`.
   - **Fit framing.** `TrackballCamera::frame()` distance = `boundingRadius /
     sin(fovY/2) * 1.05` — fits the bounding *sphere* to the FOV with ~5% margin
     (tight). Sphere (not projected extent) so the whole model stays visible in
     any orbit orientation.
...
else OBJ) with a cube fallback. The default model is `assets/plane.bdf` (a real
aircraft surface mesh: **97,754 GRID + 195,958 CTRIA3 shells**, ~25 MB),
converted from a 197,772-triangle binary STL (`assets/plane.stl`) by welding
coincident vertices into shared GRIDs and emitting one CTRIA3 per triangle.
Parse + surface-extract is <1 s; renders fine through the (still single-blocking)
renderer. Other test decks: `assets/car.bdf` (procedural generic car, 350 CHEXA →
1200 tris) and `assets/model.bdf` (8×8×2 = 128-hex block) for quick checks. The
two procedural decks are generated by ad-hoc Python (CHEXA hex boxes, shared node
map); the plane via an STL→CTRIA3 script.

> **Surface vs. shell meshes:** the STL is a *surface* (skin) mesh, so the right
> Nastran mapping is `CTRIA3` **shell** elements (one per triangle) — not solid
> CHEXA/CTETRA, which would require true volume meshing. Our pipeline renders
> shells identically (the "surface" of a shell mesh is the mesh itself).
...
The Layer-2 milestone was first hit with the 128-hex block (rectangular prism,
194 verts / 384 tris); the default has since moved to the car model (614 verts /
1200 tris), also confirmed loading/rendering. This was the **first on-screen FE
model** milestone.
